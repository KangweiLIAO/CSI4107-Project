...
We used three algorithm to obtain the similarity scores in this assignment. 
### TF-IDF
...
### Word2Vec (Gives the highest performance in our experiments)
The word2vec algorithm uses a neural network model to learn word associations from a large corpus of text. After training, 
such a model can detect synonymous words (what we need to perform query expansion) for a partial sentence with 2 avaliable methods (CBOW and Skip-gram).
word2vec represents each distinct word as a vector, and the cosine similarity of vectors indicates the level of semantic similarity 
between the words represented by those vectors.

In our experiments, I used skip-gram since the performance is better than CBOW.

Steps to perform word2vec query expansion in our experiments:
...
is the simple observation that ratios of word-word co-occurrence probabilities have the potential for encoding some form of meaning.

The training objective of GloVe is to learn word vectors such that their dot product equals the logarithm of the words' probability 
of co-occurrence. Owing to the fact that the logarithm of a ratio equals the difference of logarithms, this objective associates 
(the logarithm of) ratios of co-occurrence probabilities with vector differences in the word vector space. 
...
The Glove algorithm is similar to the word2vec algorithm, except that the computation used is different and their mods are common to some extent. 
So I modified part of the word2vec algorithm to use it. I tried many ways to integrate the training process into our code, but encountered many difficulties. 
The existing glove_python does not run well on the latest python version, and the latest supported version is python 3.8, which affects my teammate's code. 

So I can only train my model in linux environment:\
![](docs/img.jpeg)

The main training parameters are:\
![](docs/img1.jpeg)

The important ones are corpus, vocab_mincount, vector_size, window_size. corpus represents the corpus we need to use, 
in this case we need to use the processed doc dataset. while mincount indicates how many occurrences of the word will be 
vectorized and vector size is how many dimensions our vector needs to have, and finally window size indicates how many words 
are considered to be continuously associated. I have calculated our dataset and the average length is 8, so I set the window size to 8.

The computed word vectors are averaged and compared with the averaged document vectors. We used cosine similarity to compare 
the vector similarity of query and doc. I also tried the Pearson similarity method, but the improvement was not significant, 
but had a significant impact on performance. With this parametric model in the case of expanding query using three words，
Map is 0.1977 and p_10 is 0.2633. and the effect is somewhat improved by extending the query to five. map is 0.2016 and p_10 is 0.2694. 
So I tried to increase the dimensionality and window size more. I set the dimensionality to 600 and adjusted the window size to 10. 
This took almost a full ten minutes to train. And after 100 iterations, the results of the model using the five-word extension query 
were Map is 0.2015，p_10 is 0.2673。It seems that the gains of boosting dimension and window have reached their theoretical limits. 
Then I tried using glove's twitter pre-training model Twitter200. with three words extended query, Map was 0.2142 and p_10 was 0.3122.
the result was significantly better than our own training result. I tried to change the number of extensions and found that the model
worked best with 1 extension, with Map 0.2348 and p_10 0.3388. I also tried to use the largest glove model so far, Common Crawl 
(840B tokens, 2.2M vocab, cased, 300d vectors), which has The size of the model after decompression is over 5GB. After a long wait,
I got the results: Map was 0.2182 and p_10 was 0.3041. So I finally decided to use twitter200 as the final model and choose the extended word for a time case.
...
Glove does not seem to perform as well as word2vec used by my teammates, and using huge datasets (2B tweets, 27B tokens,
1.2M vocab, 200d vectors) for our application scenario does not seem to significantly improve accuracy. I reviewed some 
relevant papers online and it seems that glove has some advantage in specific cases, while many times the two are similar. 
