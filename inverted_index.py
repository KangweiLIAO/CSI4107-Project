...
class InvertedIndex:
	...
	"""
	raw_docs_dict: dict[str, str] = {}  # dict[doc_id, raw_str]
	pred_docs_dict: dict[str, list[str]] = {}  # dict[doc_id, preprocessed_tokens]
	term_dict: dict[str, list[str]] = {}  # dict[term, posting_list]

	def __init__(self, docs_path: str, save_path, read_from: str = ''):
		read_succeeded = False
		if not read_from == '':
			try:
				self.load(save_path)  # try load from local files
		...
		if not read_succeeded:
			...
			for k, v in utils.read_documents(docs_path).items():
				self.raw_docs_dict[k] = v
				self.pred_docs_dict[k] = utils.preprocess_str(v)
			print(f"{CTColors.OKGREEN}Documents preprocessing completed{CTColors.ENDC}")

			# indexing:
			for d_id, doc_content in self.pred_docs_dict.items():
			...
			self.save(save_path)  # save this inverted index object
	...
	def get_raw_tf(self, word: str, doc_id: str) -> int:
		...
		"""
		tf = 0
		for term in self.pred_docs_dict[doc_id]:
			if word == term:
				tf += 1
		return tf
	...
	def get_idf(self, word: str) -> float:
		...
		if word not in self.term_dict:
			return 0
		return math.log2(len(self.pred_docs_dict) / len(self.term_dict[word]))
	...
	def load(self, load_path):
		with open(load_path + "raw_doc_save.bin", 'rb') as file:
			self.raw_docs_dict = pickle.load(file)
			file.close()
		with open(load_path + "pred_doc_save.bin", 'rb') as file:
			self.pred_docs_dict = pickle.load(file)
			file.close()
		with open(load_path + "terms_save.bin", 'rb') as file:
			self.term_dict = pickle.load(file)
			file.close()

	def save(self, save_path):
		with open(save_path + "raw_doc_save.bin", 'wb') as file:
			pickle.dump(self.raw_docs_dict, file)
			file.close()
		with open(save_path + "pred_doc_save.bin", 'wb') as file:
			pickle.dump(self.pred_docs_dict, file)
			file.close()
		with open(save_path + "terms_save.bin", 'wb') as file:
	...
	def __str__(self):
		tmp = "Documents:\n"
		for d_id in self.pred_docs_dict:
			tmp += str(self.pred_docs_dict[d_id]) + "; "
		tmp += "\nTerms:\n"
