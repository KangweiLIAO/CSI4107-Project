...
def train_d2v_model(docs_dict):
	print("Training Doc2Vec model... (~= 65s)")
	start_time = time.time()
	# Prepare training set
	train_set = [TaggedDocument(docs_dict[key], tags=[key]) for key in docs_dict]
	model = Doc2Vec(vector_size=300, dm=1, alpha=0.1, window=3, workers=multiprocessing.cpu_count(), epochs=40)
...
def similarity_scores(index: IRIndex, queries: list[(str, list[str])], doc_per_query=1000):
	# Load Model
	...
	for pair in queries:  # (q_id, raw_query)
		query = pair[1].split()
		inferred_vector = model.infer_vector(query, alpha=0.025, epochs=model.epochs)
