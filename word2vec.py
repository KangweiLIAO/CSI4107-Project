...
def train_w2v_model(train_data: dict[str, list[str]], sg=1) -> Word2Vec:
	...
	"""
	print("Training Word2Vec model... (~= 30s)")
	start_time = time.time()
	train_data = [words for words in train_data.values()]
	# train a skip-gram model:
	model = Word2Vec(sentences=train_data, vector_size=300, alpha=0.1, min_count=2, window=15, sg=sg, epochs=45,
...
def similarity_scores(inv_index: IRIndex, queries: list[(str, list[str])], topn=3, doc_per_query=1000):
	...
	"""
	model = train_w2v_model(inv_index.docs_dict).wv
